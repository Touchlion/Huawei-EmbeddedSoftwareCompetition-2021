...
int type[50005];//0表示基站，1表示未决定类型的卫星，2表示中转卫星，3表示接收卫星
int id[50005]; 
LL total_dis = 0; 
struct Node//邻接点
{
...
vector<Node>G[60000];//用邻接表实现图
vector<int>route[60000];
...
int main()
{
	freopen("training-2.txt", "r", stdin); //需要读入同一测试用例
	cin >> N >> E >> C >> D >> PS;
	for (int i = 0; i < N; i++)
	{
		cin >> type[i];
	...
	freopen("out.txt", "r", stdin); //读入程序输出的方案
	memset(id, 0, sizeof(id));
	int a0, a1; char ch;
	int row = 1;
	while (cin >> a0) //每行第一个数是基站id
	{
	...
	cout << "总功耗为 " << total_dis*C + count_sat*PS << "\n";
}